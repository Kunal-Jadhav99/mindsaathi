...
function EmptyChart({ message }) {
  return (
    <div style={{
      ...
      flexDirection: 'column', gap: '10px', color: '#9090a8',
    }}>
      <TrendingUp size={32} style={{ opacity: 0.3 }} />
      <p style={{ fontSize: '13px', textAlign: 'center' }}>{message}</p>
    </div>
  );
}
  ...
  async function loadDashboardData() {
    ...
    try {
      ...
      if (
        ...
        setError('Could not connect to the backend. Check your API configuration.');
      }
    } catch (err) {
    ...
    <div className="page animate-fade-in">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="card-elevated">
          ...
          {loading ? (
            <EmptyChart message="Loading trend data…" />
          ) : weeklyTrend.length === 0 ? (
            <EmptyChart message="No weekly trend data yet. Data will appear once students complete check-ins." />
          ) : (
        ...
        <div className="card-elevated">
          ...
          {loading ? (
            <EmptyChart message="Loading department data…" />
          ) : deptStats.length === 0 ? (
            <EmptyChart message="No department data yet. Data will appear once check-ins are submitted." />
          ) : (
            <ResponsiveContainer width="100%" height={300}>
